...
/**
 * useSupsRefresh - custom hook for Super Admin pages to trigger data re-fetching
 ...
 */
export function useSupsRefresh(fetchFn) {
  ...
  const RefreshOverlay = isRefreshing ? (
    <GlobalLoader title="Skill Sups" logo={supIcon} />
  ) : null;
  ...
  const RefreshButton = (
    <button
      ...
      className="p-2.5 bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-600 rounded-xl transition-all cursor-pointer shadow-xs hover:scale-105 active:scale-95 flex items-center justify-center disabled:opacity-60"
    >
      <svg
